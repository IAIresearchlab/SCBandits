...
def hist_and_cutoffs(df = None, to_check = None, to_check_unif = None, to_check_ipw = None, n = None, num_sims = None, load_df = True, \
                     title = None, plot = True, to_check_ipw_mean1 = None, to_check_ipw_mean2 = None, reward_header = None, condition_header = None, center = None):
    '''
...
#changed from Uniform for debugging
            ...
            ax.hist(wald_type_stat, \
              ...
              label = "\n TS synthetic means for cutoffs: \n $\mu$ = {} \n $\sigma$ = {} \n bias($\hatp_1$) = {} \n bias($\hatp_2$) = {}".format(
                      ...
                      np.round(mle_mean1 - center,3), np.round(mle_mean2 - center,3)
                      )
              )
...
#             transform = ax[i].transAxes)
    ...
    if plot == True:
        ...
        fig.tight_layout(rect=[0, 0.03, 1, 0.80])
         ...
         #   os.path.mkdir("plots")
        print("saving to ", "Centers_varying/Histograms_unif_isrealTS/{}.png".format(title))
        fig.savefig("Centers_varying/Histograms_unif_isrealTS/{}.png".format(title))
 ...
 #   print(ind)
  #  print(step_sizes)
   ...
   # autolabel(p1, ax)
    ...
    p2 = ax.bar(ind + bs_step, ipw_t1_list, width = width, yerr = t1_ipw_se, \
                ecolor='green', capsize=capsize, \
                alpha = alpha_bs, color = 'green', hatch = hatch_bs, edgecolor='black') #ind+2*width_total + bs_step
   
    #prev_ind + total_width + width_prev/2 + width_curr
    p3 = ax.bar(ind-width_total + bs_step, t1_wald_list, width =width, \
                yerr = t1_wald_se, ecolor='black', \
                capsize=capsize, alpha = alpha_bs, \
                color = 'blue', edgecolor='black', hatch = hatch_bs) #ind + bs_step
    ...
    p5 = ax.bar(ind+2*width_total + bs_step, t1_simbased_list_ipw, width = width,\
                ...
                edgecolor='black', alpha = alpha_bs, hatch = hatch_bs)#ind + bs_step
...
#    if ax_idx == 6 or ax_idx == 7 or ax_idx == 8:
    ...
    return [t1_wald_list_unif[0], t1_wald_list[0], t1_simbased_list[0], ipw_t1_list[0]] #returns [UR TS TSEC IPW], also en  lists so take [0]
...
def parse_dir(root, root_cutoffs, num_sims, outcomes_title_dict, sample_size_dict, experiment_title, condition_header):
    ...
    for outcome in outcomes_list: #one plot per outcome, and also one table
            ...
            for center in ["0pt25","0pt5","0pt75"]:
                ...
                for bs_prop in bs_prop_list:
                    ...
                    title = "Cutoffs From $p1 = p2 = {}$ \n Experiment: {} \n Outcome: {} \n  n = {} and {} sims \n Batch Size {} \n $p_0 = {}, p_1 = {}, |p_0 - p_1| = {}$".format(float(center.replace("pt",".")), experiment_title, outcomes_title_dict[outcome], n, num_sims, bs, p0, p1, delta)
                    
                    percentile_dict_left, percentile_dict_right, \
                    percentile_dict_left_ipw, percentile_dict_right_ipw, \
                     cell_str = hist_and_cutoffs(to_check = to_check_cutoffs, to_check_unif = to_check, to_check_ipw = to_check_ipw_cutoffs,\
    		...
    		#			       ax = ax[i], bs_prop = bs_prop, ax_idx = i, outcomes_list = outcomes_list, reward_header = outcome, condition_header = condition_header)
                    ...
                    next_col = stacked_bar_plot_with_cutoff(to_check = to_check,to_check_unif = to_check_unif, to_check_ipw = ipw_t1,\
					      n = n, num_sims = num_sims, \
					       ...
					       ax = ax[i], bs_prop = bs_prop, ax_idx = i, \
                ...
                df.index = ["Uniform Random","Thompson Sampling", "Thompson Sampling \n With Empirical Cutoffs", "IPW"]
                df.to_csv("Tables/Power_For_{}_Experiment_With_{}_Outcome_p0_{}_p1_{}_n_{}_Center_{}.csv".format(experiment_title, outcome, str(p0).replace(".","pt"), str(p1).replace(".","pt"), n, center)) 

                ax[i].set_title("Cutoffs From $p_1 = p_2 = {}$".format(float(center.replace("pt","."))))
 ...
 #df["Cutoffs From $p0 = p2 = {}$ \n PCRS Data with $p_0 = {}, p_1 = {}, |p_0 - p_1| = {}$".format(float(center.replace("pt",".")), p0, p1, delta)] = col
          ...
          #  df.to_csv("Cutoffs/Cutoffs For_{}_Experiment_With_{}_Outcome_p0_{}_p1_{}_n_{}.csv".format(experiment_title, outcomes_title_dict[outcome], str(p0).replace(".","pt"), str(p1).replace(".","pt"), n))
           ...
           # title_save = "Power Across {} Simulations For {} Experiment With {} Outcome p_0 = {}, p_1 = {}, |p_0 - p_1| = {} Number of Participants = {}".format(num_sims, experiment_title, outcomes_title_dict[outcome], p0, p1, delta, n)
        ...
        #fig.tight_layout()
         ...
         #   os.mkdir("plots_multiarm_bs")
            print("saving to ", "Centers_varying/{}.png".format(title_save))
            fig.savefig("Centers_varying/{}.png".format(title_save), bbox_inches = 'tight')
            plt.show()
            plt.clf()
            plt.close()

...
#def probability_2_beats_1(sucesses_1, sucesses_2, failures_1, failures_2)
# 
#    #1 - > A, 2 - > B
#    sucesses_1 += 1
#    sucesses_2 += 1
...
#
#    total = 0.0
#    for i in range(len(sucesses_2)):
#        total += exp(logbeta(α_A+i, β_B+β_A)
#                                - log(β_B+i) - logbeta(1+i, β_B) - logbeta(α_A, β_A))
#    return total
#
#def compute_probability_condition_assignment(num_actions, models):
#    sucesses = [models[a].successes for a in range(num_actions)]
#    if num_samples > 0:
#       # samples = [models[a].draw_expected_value(context, num_samples) for a in range(num_actions)]
#        samples = [models[a].draw_expected_value(num_samples = num_samples) for a in range(num_actions)]
#    
#        ## generate a matrix of size (num_actions x num_trials) containing sampled expected values
#        samples = np.array(samples)
#    

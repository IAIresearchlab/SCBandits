        ...
        if plot == True:
            ...
            ax.hist(wald_type_stat, \
              ...
              label = "\n Thompson Sampling: \n $\mu$ = {} \n $\sigma$ = {} \n bias($\hatp_1$) = {} \n bias($\hatp_2$) = {}".format(
                      ...
                      np.round(mle_mean1 - p1,3), np.round(mle_mean2 - p2,3)
                      )
              )
...
#                edgecolor='black', alpha = alpha_bs, hatch = hatch_bs)
    ...
    if ax_idx == 0:
        leg1 = ax.legend((p1[0], p3[0], p4[0]), ("Simulation Based Emp. Cut-off Wald", \
                   'Thompson Wald', "Uniform Wald"), loc = "upper left", bbox_to_anchor=(-0.01, 1.25))
        ax.add_artist(leg1)
...
def parse_dir(root, root_cutoffs, num_sims, outcomes_title_dict, sample_size_dict, experiment_title, condition_header):
    ...
    #ax = ax.ravel()
    outcomes_list = ["Y4"] #348 640 460 657
    ...
    for outcome in outcomes_list:
            ...
            for bs_prop in bs_prop_list:
                ...
                title = "Experiment {} Outcomes = {} \n  n = {} and {} sims \n Initial Batch Size {} and Batch Size {}".format(experiment_title, outcomes_title_dict[outcome], n, num_sims, bs, bs)
                ...
                stacked_bar_plot_with_cutoff(to_check = to_check,to_check_unif = to_check_unif,\
					      n = n, num_sims = num_sims, \
					       ...
					       ax = ax, bs_prop = bs_prop, ax_idx = i, outcomes_list = outcomes_list, reward_header = outcome, condition_header = condition_header)
	       
            ax.set_title("Outcome: {} \n $p_0 = {}, p_1 = {}, |p_0 - p_1| = {}$".format(outcomes_title_dict[outcome], p0, p1, delta))
            #ax.set_title("Outcome: {}".format(outcomes_title_dict[outcome]))
            i += 1

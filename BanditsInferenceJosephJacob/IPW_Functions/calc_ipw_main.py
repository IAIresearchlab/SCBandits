...
sys.path.insert(1, '../../banditalgorithms/src/louie_experiments/')
...
def calculate_ipw_save(actions_df, num_samples, num_actions = 2, cached_probs={}, 
                  prior = [1,1], binary_rewards = True, config = {}):
    """
    saves if finds nans for success probs
    """
    assert num_actions == 2
    read_config.apply_defaults(config)
...
#MAX_WEIGHT = float("inf")
   ...
   # print("actions_df", actions_df)
    ...
    return ipws, saved_probs_1

def calculate_ipw_stats(actions_df, num_samples, num_actions = 2, cached_probs={}, 
                  prior = [1,1], binary_rewards = True, config = {}):
    """
    saves if finds nans for success probs
    """
   ...
   # print("AD cols in ipw", actions_df.columns)
    assert sample_size_1 == (actions_df["AlgorithmAction"] == 1).sum()
    ...
    return mean_1_ipw, mean_2_ipw, wald_pval_ipw, wald_type_stat_ipw



def calculate_ipw_by_step_size(actions_df, num_samples, num_actions = 2, cached_probs={}, 
...
#         print("Calculated nobs for effect size:", nobs_total)
 #       n = math.ceil(nobs_total)
  #      step_sizes = [math.ceil(n/2), n, 2*n, 4*n]
   ...
   # step_sizes = [n]
    i=0
    calculate_ipw_save(actions_df, num_samples, num_actions = 2, cached_probs={}, 
        prior = [1,1], binary_rewards = True, config = {})
#    for num_steps in step_sizes:
...
def parse_dir(root, root_cutoffs, num_sims, outcomes_title_dict, sample_size_dict, experiment_title, condition_header, effect):
    #num_sims = 1000
   ...
   # fig.set_size_inches(17.5, 13.5)
    ...
    for outcome in outcomes_list:
            ...
            for bs_prop in bs_prop_list:
                ...
                actions_root = outcome_dir + "/bbEqualMeansEqualPriorburn_in_size-batch_size={}-{}".format(bs,bs)
                ...
                calculate_ipw_by_step_size(actions_df = actions_root, num_samples=1000, num_actions = 2, cached_probs = {}, \
...
if __name__ == "__main__":
    ...
    parse_dir(root, root_cutoffs, num_sims, outcomes_title_dict, sample_size_dict, experiment_title, condition_header, effect = True)
    ...
    #parse_dir(root, root_cutoffs, num_sims, outcomes_title_dict, sample_size_dict, experiment_title, condition_header, effect = False)
    #num_sims=5000
    #Extra Problems-----------
...
#     actions_root_32 = "/Users/jacobnogas/Documents/School/Summer_2019/IPW Sofia Villar/banditalgorithms/src/From_Annas_server/Aug_18_vac_sims/2019-08-17_19:58:28num_sims1000init_batch_size-batch_size=10-10/bbEqualMeansEqualPrior32init_batch_size-batch_size=10-10"
    
   
#     data_root = "/Volumes/Backup/Summer2019/IPW_Bandits/Aug_18_vac_sims"

#     data_root = '../Outfile/ParamsBased/numSims1000motivate_finalRepeatForCutoffs/'
#     num_sims = 1000
#     n_list = [32, 88, 785]
#    # n_list = [32]
...
#         #BS 10---
#         bs = 10
#         actions_root = data_root + "/2019-08-17_19:58:28num_sims1000init_batch_size-batch_size={}-{}/bbEqualMeansEqualPrior{}init_batch_size-batch_size={}-{}".format(bs,bs,n, bs,bs)
#        # test_actions = actions_root + "/tbb_actions_{}_{}.csv".format(n, sim_count)
...
#         calculate_ipw_by_step_size(actions_root = actions_root, num_samples=1000, num_actions = 2, cached_probs = {}, \
#                       prior = prior, binary_rewards = is_binary, config = config, n = n, num_sims = num_sims, batch_size = bs)
#         #BS 30-----
#         bs = 30
#         actions_root = data_root + "/2019-08-17_19:59:38num_sims1000init_batch_size-batch_size={}-{}/bbEqualMeansEqualPrior{}init_batch_size-batch_size={}-{}".format(bs,bs,n, bs,bs)
#        # test_actions = actions_root + "/tbb_actions_{}_{}.csv".format(n, sim_count)
...
#         #BS 10---
#         bs = 10
#         actions_root = data_root + "/IsEFFECT2019-08-17_20:07:20num_sims1000init_batch_size-batch_size={}-{}/bb{}init_batch_size-batch_size={}-{}".format(bs,bs,es, bs,bs)
#        # test_actions = actions_root + "/tbb_actions_{}_{}.csv".format(n, sim_count)
...
#         calculate_ipw_by_step_size(actions_root = actions_root, num_samples=1000, num_actions = 2, cached_probs = {}, \
#                       prior = prior, binary_rewards = is_binary, config = config,\
#                       n = n, num_sims = num_sims, batch_size = bs, effect_size = es, no_effect = False)
#         #BS 30-----
#         bs = 30
        
#         actions_root = data_root + "/IsEFFECT2019-08-17_20:14:39num_sims1000init_batch_size-batch_size={}-{}/bb{}init_batch_size-batch_size={}-{}".format(bs,bs,es, bs,bs)
#        # test_actions = actions_root + "/tbb_actions_{}_{}.csv".format(n, sim_count)

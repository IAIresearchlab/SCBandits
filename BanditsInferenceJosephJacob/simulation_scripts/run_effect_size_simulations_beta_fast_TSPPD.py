...
sys.path.insert(1, '../IPW_Functions/')
...
def make_stats_row_from_df(simulations_df, include_power, action_count, effect_size = None, alpha = None, step_size = "JEDM"):
    ...
    if step_size == "JEDM":
        ...
        step_size = max(simulations_df.index) + 1 #will be 4n for instance
        n_size_list = np.arange(2, int(step_size/4 + 1))#2 to 4*n 
    ...
    for n_size in n_size_list:
        sim_num = 0
        for idx in range(0, trials_count, step_size): #going through sims
            one_sim_df = simulations_df[idx:idx + n_size].copy()
            assert(len(one_sim_df) == n_size)

            cur_row = {}#one row per sim
...
#            print("n_size", n_size, exploring_this_n)
            ...
            mean_1_ipw, mean_2_ipw, wald_pval_ipw, wald_type_stat_ipw = calculate_ipw_stats(actions_df = one_sim_df, num_samples = 1000, num_actions = 2, cached_probs={}, prior = prior, binary_rewards = is_binary, config = config)

            cur_row['mean_1_ipw'] = mean_1_ipw
            cur_row['mean_2_ipw'] = mean_2_ipw
            cur_row['wald_pval_ipw'] = wald_pval_ipw
            cur_row['wald_type_stat_ipw'] = wald_type_stat_ipw
    #        cur_row['num_steps'] = n_size 
            ...
            #add in prob act 1 is best at the sample size "SampleNumber" == ..
            cur_row["arm1_assignment_prob"] = one_sim_df[H_ALGO_PROB_BEST_ACTION.format(1)].iloc[-1]
          #  curr_row[]
          #  H_ALGO_PROB_BEST_ACTION.format(1)
...
def calculate_statistics_from_sims(simulations_dfs_list, effect_size,
    ...
    columns=['num_steps', 'sim', 'sample_size_1', 'sample_size_2', 'mean_1',
        ...
        'wald_pval', 'wald_type_stat', "prop_exploring_ppd_cuml", "exploring_ppd_at_this_n", "mean_1_ipw", "mean_2_ipw", "wald_pval_ipw", "wald_type_stat_ipw", "arm1_assignment_prob"]
...
def run_simulations(num_sims, prob_per_arm, step_sizes, outfile_directory,
    ...
    for num_steps in step_sizes:
        ...
        for i in range(num_sims):
            ...
            sim_result_df = pd.DataFrame(sim_result, columns=column_names) #used for ipw
#            print("sim_result_df", sim_result_df)
            ...
            ipw_col, ap_col = calculate_ipw_save(actions_df = sim_result_df, num_samples = 1000, num_actions = 2, cached_probs={}, prior = prior, binary_rewards = is_binary, config = config)

            for i in range(len(sim_result)):
                sim_result[i].append(ipw_col[i]) 
                sim_result[i].append(ap_col[i]) 

            sim_results.extend(sim_result)

        column_names.append(IPWS_COL)
        column_names.append(H_ALGO_PROB_BEST_ACTION.format(1))
        #NOTE AP IS NOTE SAVED, onlt thing in makestatsrow are saved
 ...
 #   if len(sys.argv) > 5:
  #      epsilon = float(sys.argv[5])
   #     print("epsilon", epsilon)
    ...
    if effect_size == 0 and n == 657:
        n = 657/4
        print("using real world sample size", n)
    ...
    print("len(sys.argvs)", len(sys.argv))
    print("sys.argv[5]", print(sys.argv[5]))
    prior_params = None
        ...
        if bandit_type == "uniform":
            ...
            if len(sys.argv) > 5:
                if sys.argv[5] == "armsHigh":
                    ...
                    prior_params = [priorProportionOnSuccess*2, 2-priorProportionOnSuccess*2]
                elif "JeffPrior" in outfile_directory:
